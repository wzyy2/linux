...
static int rk_init(struct snd_soc_pcm_runtime *runtime)
{
	...
	ret = snd_soc_card_jack_new(codec->component.card, "Mic Jack",
			       SND_JACK_HEADPHONE,
			       &mic_jack, NULL, 0);
...
static int snd_rk_mc_gpio_init(struct platform_device *pdev)
{
	...
	hpdet_gpiod = devm_gpiod_get(&pdev->dev, "hpdet", GPIOD_OUT_LOW);
	...
	micdet_gpiod = devm_gpiod_get(&pdev->dev, "micdet", GPIOD_OUT_LOW);
