...
static void rga_cmd_set_alpha_blend(struct rga_ctx *ctx)
{
	...
	alpha_ctrl0.val = ctx->alpha0;
	alpha_ctrl1.val = ctx->alpha1;
...
void rga_cmd_set(struct rga_ctx *ctx, void *src_mmu_pages, void *dst_mmu_pages)
{
	...
	if (ctx->op == OP_ALPHA_BLEND) {
		/*
		 * Due to hardware bug,
		 * src1 mmu also should be configured when use alpha blending.
		 */
