...
#define TC358749xbg_FETCH_LSB_GAIN(VAL) (VAL & 0x00FF)       /* gain[7:0] */
#define TC358749xbg_FETCH_MSB_GAIN(VAL) ((VAL >> 8) & 0x1)	/* gain[10:8] */
#define TC358749xbg_AEC_PK_LONG_GAIN_HIGH_REG 0x350a	/* Bit 8 */
#define TC358749xbg_AEC_PK_LONG_GAIN_LOW_REG	 0x350b	/* Bits 0 -7 */

#define TC358749xbg_AEC_PK_LONG_EXPO_3RD_REG 0x3500	/* Exposure Bits 16-19 */
#define TC358749xbg_AEC_PK_LONG_EXPO_2ND_REG 0x3501	/* Exposure Bits 8-15 */
#define TC358749xbg_AEC_PK_LONG_EXPO_1ST_REG 0x3502	/* Exposure Bits 0-7 */

#define TC358749xbg_AEC_GROUP_UPDATE_ADDRESS 0x3212
#define TC358749xbg_AEC_GROUP_UPDATE_START_DATA 0x00
#define TC358749xbg_AEC_GROUP_UPDATE_END_DATA 0x10
#define TC358749xbg_AEC_GROUP_UPDATE_END_LAUNCH 0xA0

#define TC358749xbg_FETCH_3RD_BYTE_EXP(VAL) (((VAL) >> 12) & 0xF)	/* 4 Bits */
#define TC358749xbg_FETCH_2ND_BYTE_EXP(VAL) (((VAL) >> 4) & 0xFF)	/* 8 Bits */
#define TC358749xbg_FETCH_1ST_BYTE_EXP(VAL) (((VAL) & 0x0F) << 4)	/* 4 Bits */

#define TC358749xbg_PID_ADDR     0x0000

/* product ID */
#define TC358749xbg_PID_MAGIC 0x0147

#define TC358749xbg_EXT_CLK 24000000
#define TC358749xbg_PLL_PREDIV0_REG 0x3088
#define TC358749xbg_PLL_PREDIV_REG  0x3080
#define TC358749xbg_PLL_MUL_HIGH_REG 0x3081
#define TC358749xbg_PLL_MUL_LOW_REG 0x3082
#define TC358749xbg_PLL_SPDIV_REG 0x3086
#define TC358749xbg_PLL_DIVSYS_REG 0x3084
#define TC358749xbg_TIMING_VTS_HIGH_REG 0x380e
#define TC358749xbg_TIMING_VTS_LOW_REG 0x380f
#define TC358749xbg_TIMING_HTS_HIGH_REG 0x380c
#define TC358749xbg_TIMING_HTS_LOW_REG 0x380d
#define TC358749xbg_FINE_INTG_TIME_MIN 0
#define TC358749xbg_FINE_INTG_TIME_MAX_MARGIN 0
#define TC358749xbg_COARSE_INTG_TIME_MIN 1
#define TC358749xbg_COARSE_INTG_TIME_MAX_MARGIN 4
#define TC358749xbg_TIMING_X_INC		0x3814
#define TC358749xbg_TIMING_Y_INC		0x3815
#define TC358749xbg_HORIZONTAL_START_HIGH_REG 0x3800
#define TC358749xbg_HORIZONTAL_START_LOW_REG 0x3801
#define TC358749xbg_VERTICAL_START_HIGH_REG 0x3802
#define TC358749xbg_VERTICAL_START_LOW_REG 0x3803
#define TC358749xbg_HORIZONTAL_END_HIGH_REG 0x3804
#define TC358749xbg_HORIZONTAL_END_LOW_REG 0x3805
#define TC358749xbg_VERTICAL_END_HIGH_REG 0x3806
#define TC358749xbg_VERTICAL_END_LOW_REG 0x3807
#define TC358749xbg_HORIZONTAL_OUTPUT_SIZE_HIGH_REG 0x3808
#define TC358749xbg_HORIZONTAL_OUTPUT_SIZE_LOW_REG 0x3809
#define TC358749xbg_VERTICAL_OUTPUT_SIZE_HIGH_REG 0x380a
#define TC358749xbg_VERTICAL_OUTPUT_SIZE_LOW_REG 0x380b
#define TC358749xbg_H_WIN_OFF_HIGH_REG 0x3810
#define TC358749xbg_H_WIN_OFF_LOW_REG 0x3811
#define TC358749xbg_V_WIN_OFF_HIGH_REG 0x3812
#define TC358749xbg_V_WIN_OFF_LOW_REG 0x3813

#define TC358749xbg_ANA_ARRAR01 0x3621
#define TC358749xbg_TIMING_CONCTROL_VH 0x3803
#define TC358749xbg_TIMING_CONCTROL18 0x3818
...
static struct tc_camera_module_reg tc358749xbg_init_tab_1920_1080_30fps[] = {
	   //<!-- Software Reset -->
...
/*static struct tc_camera_module_reg tc358749xbg_reg_video_on[] = {
	//Let HDMI Source start access
	{TC_CAMERA_MODULE_REG_TYPE_DATA, 0x854A,0x00, 1},
	//Let HDMI Source start access
	{TC_CAMERA_MODULE_REG_TYPE_DATA, 0x854A,0x00, 1},

	{TC_CAMERA_MODULE_REG_TYPE_TIMEOUT, 0x0000,0x1, 1},

	//Wait until HDMI sync is established
	{TC_CAMERA_MODULE_REG_TYPE_DATA, 0x8520,0x00, 1},//CLK_Status
	{TC_CAMERA_MODULE_REG_TYPE_DATA, 0x0004,0x0400, 2},//ConfCtl 0CD7
	{TC_CAMERA_MODULE_REG_TYPE_DATA, 0x0000,0x00, 1},
};
*/
/* ======================================================================== */
static struct tc_camera_module_config tc358749xbg_configs[] = {
	{
		...
		.reg_table = (void *)tc358749xbg_init_tab_1920_1080_30fps,
		.reg_table_num_entries =
			sizeof(tc358749xbg_init_tab_1920_1080_30fps) /
			sizeof(tc358749xbg_init_tab_1920_1080_30fps[0]),
...
static int tc358749xbg_g_fmt(struct v4l2_subdev *sd,
		...
		struct v4l2_subdev_format *format)
{
	...
	pltfrm_camera_module_pr_err(&cam_mod->sd, "read VI_REP return 0x%x\n", vi_rep);
...
/*
static int tc358749xbg_s_fmt(struct v4l2_subdev *sd,
		struct v4l2_subdev_pad_config *cfg,
		struct v4l2_subdev_format *format)
{
	...
	int ret = tc358749xbg_g_fmt(sd, cfg, format);
...
static int tc358749xbg_filltimings(struct tc_camera_module_custom_config *custom)
{
	int i, j;
	...
	if (0) {
		...
		for (j = 0; j < reg_table_num_entries; j++) {
			...
			case TC358749xbg_TIMING_VTS_HIGH_REG:
				timings->frame_length_lines =
					((reg_table[j].val << 8) |
					(timings->frame_length_lines & 0xff));
				break;
			case TC358749xbg_TIMING_VTS_LOW_REG:
				timings->frame_length_lines =
					(reg_table[j].val |
					(timings->frame_length_lines & 0xff00));
				break;
			case TC358749xbg_TIMING_HTS_HIGH_REG:
				timings->line_length_pck =
					((reg_table[j].val << 8) |
					timings->line_length_pck);
				break;
			case TC358749xbg_TIMING_HTS_LOW_REG:
				timings->line_length_pck =
					(reg_table[j].val |
					(timings->line_length_pck & 0xff00));
				break;
			case TC358749xbg_TIMING_X_INC:
				timings->binning_factor_x =
					((reg_table[j].val >> 4) + 1) / 2;
				if (timings->binning_factor_x == 0)
					timings->binning_factor_x = 1;
				break;
			case TC358749xbg_TIMING_Y_INC:
				timings->binning_factor_y =
					((reg_table[j].val >> 4) + 1) / 2;
				if (timings->binning_factor_y == 0)
					timings->binning_factor_y = 1;
				break;
			case TC358749xbg_HORIZONTAL_START_HIGH_REG:
				timings->crop_horizontal_start =
					((reg_table[j].val << 8) |
					(timings->crop_horizontal_start &
					0xff));
				break;
			case TC358749xbg_HORIZONTAL_START_LOW_REG:
				timings->crop_horizontal_start =
					(reg_table[j].val |
					(timings->crop_horizontal_start &
					0xff00));
				break;
			case TC358749xbg_VERTICAL_START_HIGH_REG:
				timings->crop_vertical_start =
					((reg_table[j].val << 8) |
					(timings->crop_vertical_start & 0xff));
				break;
			case TC358749xbg_VERTICAL_START_LOW_REG:
				timings->crop_vertical_start =
					((reg_table[j].val) |
					(timings->crop_vertical_start &
					0xff00));
				break;
			case TC358749xbg_HORIZONTAL_END_HIGH_REG:
				timings->crop_horizontal_end =
					((reg_table[j].val << 8) |
					(timings->crop_horizontal_end & 0xff));
				break;
			case TC358749xbg_HORIZONTAL_END_LOW_REG:
				timings->crop_horizontal_end =
					(reg_table[j].val |
					(timings->crop_horizontal_end &
					0xff00));
				break;
			case TC358749xbg_VERTICAL_END_HIGH_REG:
				timings->crop_vertical_end =
					((reg_table[j].val << 8) |
					(timings->crop_vertical_end & 0xff));
				break;
			case TC358749xbg_VERTICAL_END_LOW_REG:
				timings->crop_vertical_end =
					(reg_table[j].val |
					(timings->crop_vertical_end & 0xff00));
				break;
			case TC358749xbg_H_WIN_OFF_HIGH_REG:
				win_h_off = (reg_table[j].val & 0xf) << 8;
				break;
			case TC358749xbg_H_WIN_OFF_LOW_REG:
				win_h_off |= (reg_table[j].val & 0xff);
				break;
			case TC358749xbg_V_WIN_OFF_HIGH_REG:
				win_v_off = (reg_table[j].val & 0xf) << 8;
				break;
			case TC358749xbg_V_WIN_OFF_LOW_REG:
				win_v_off |= (reg_table[j].val & 0xff);
				break;
			case TC358749xbg_HORIZONTAL_OUTPUT_SIZE_HIGH_REG:
				timings->sensor_output_width =
					((reg_table[j].val << 8) |
					(timings->sensor_output_width & 0xff));
				break;
			case TC358749xbg_HORIZONTAL_OUTPUT_SIZE_LOW_REG:
				timings->sensor_output_width =
					(reg_table[j].val |
					(timings->sensor_output_width &
					0xff00));
				break;
			case TC358749xbg_VERTICAL_OUTPUT_SIZE_HIGH_REG:
				timings->sensor_output_height =
					((reg_table[j].val << 8) |
					(timings->sensor_output_height & 0xff));
				break;
			case TC358749xbg_VERTICAL_OUTPUT_SIZE_LOW_REG:
				timings->sensor_output_height =
					(reg_table[j].val |
					(timings->sensor_output_height &
					0xff00));
		...
		timings->coarse_integration_time_min =
			TC358749xbg_COARSE_INTG_TIME_MIN;
		timings->coarse_integration_time_max_margin =
			TC358749xbg_COARSE_INTG_TIME_MAX_MARGIN;

		/* TC Sensor do not use fine integration time. */
		timings->fine_integration_time_min =
			TC358749xbg_FINE_INTG_TIME_MIN;
		timings->fine_integration_time_max_margin =
			TC358749xbg_FINE_INTG_TIME_MAX_MARGIN;
	}
...
static int tc358749xbg_g_timings(struct tc_camera_module *cam_mod,
			    struct tc_camera_module_timings *timings)
{
	int ret = 0;
	unsigned int vts;
...
static int tc358749xbg_s_ext_ctrls(struct tc_camera_module *cam_mod,
...
/*static long tc358749xbg_ioctl(struct v4l2_subdev *sd,
	unsigned int cmd,
	void *arg)
{
	int ret = 0;
	...
	pltfrm_camera_module_pr_err(&cam_mod->sd, "cmd: 0x%x\n", cmd);
	...
	default:
		pltfrm_camera_module_pr_err(&cam_mod->sd, "cmd: 0x%x is not support\n", cmd);
		break;
	}
...
static int tc358749xbg_check_camera_id(struct tc_camera_module *cam_mod)
{
	...
	tc_camera_module_pr_debug(cam_mod, "test\n");

	test_value = tc_camera_module_read16_reg(cam_mod, 0x0004);
	tc_camera_module_pr_err(cam_mod,
				"read 0x0004 return 0x%x\n",
				test_value);

	tc_camera_module_write8_reg(cam_mod, 0x8540, 0x8c);
	test_value = tc_camera_module_read8_reg(cam_mod, 0x8540);
	tc_camera_module_pr_err(cam_mod,
				"read 0x8540 return 0x%x\n",
				test_value);
	tc_camera_module_write16_reg(cam_mod, 0x0004, 0xa855);
	test_value = tc_camera_module_read16_reg(cam_mod, 0x0004);
	tc_camera_module_pr_err(cam_mod,
				"read 0x0004 return 0x%x\n",
				test_value);
	...
	return 0;
err:
	tc_camera_module_pr_err(cam_mod, "failed with error (%d)\n", ret);
	return 0;
}
...
long tc_camera_358749xbg_module_ioctl(struct v4l2_subdev *sd,
	unsigned int cmd,
	void *arg)
{
	return 0;
...
struct tc358749x_priv test_priv;

static int test_parse_dts(struct i2c_client *client)
{
	//int ret = 0;
	...
	if (!np)
		printk(KERN_ERR "np is NULL");

	test_priv.gpio_int = devm_gpiod_get_optional(dev, "int",
						     GPIOD_OUT_HIGH);

	/* I2C Slave Address selection through boot-strap  */
	printk(KERN_ERR "%s(%d): gpio_int 0", __func__, __LINE__);
	gpiod_direction_output(test_priv.gpio_int, 0);


	test_priv.gpio_power = devm_gpiod_get_optional(dev, "power",
							GPIOD_OUT_HIGH);
	printk(KERN_ERR "%s(%d): gpio_power 1", __func__, __LINE__);
	gpiod_direction_output(test_priv.gpio_power, 1);

	test_priv.gpio_power18 = devm_gpiod_get_optional(dev, "power18",
							  GPIOD_OUT_HIGH);
	printk(KERN_ERR "%s(%d): gpio_power18 1", __func__, __LINE__);
	gpiod_direction_output(test_priv.gpio_power18, 1);

	test_priv.gpio_power33 = devm_gpiod_get_optional(dev, "power33",
							  GPIOD_OUT_HIGH);
	printk(KERN_ERR "%s(%d): gpio_power33 1", __func__, __LINE__);
	gpiod_direction_output(test_priv.gpio_power33, 1);

	test_priv.gpio_csi_ctl = devm_gpiod_get_optional(dev, "csi-ctl",
							  GPIOD_OUT_LOW);
	printk(KERN_ERR "%s(%d): gpio_cis_ctl 0", __func__, __LINE__);
	gpiod_direction_output(test_priv.gpio_csi_ctl, 0);

	test_priv.gpio_reset = devm_gpiod_get_optional(dev, "reset",
							GPIOD_OUT_HIGH);
	printk(KERN_ERR "%s(%d): gpio_reset 1", __func__, __LINE__);
	gpiod_direction_output(test_priv.gpio_reset, 1);

	test_priv.gpio_stanby = devm_gpiod_get_optional(dev, "stanby",
							 GPIOD_OUT_LOW);
	printk(KERN_ERR "%s(%d): gpio_stanby 1", __func__, __LINE__);
	gpiod_direction_output(test_priv.gpio_stanby, 1);

	/* Wait 10ms tc358749x lock I2C Slave address */
	usleep_range(10000, 11000);
	/* after I2C address has been lock and set it input */
	gpiod_direction_input(test_priv.gpio_int);


	printk(KERN_ERR "%s(%d): parse dts and power on", __func__, __LINE__);
	return 0;
}

static ssize_t tc358749xbg_debugfs_power_control(struct file *file,
					...
					size_t count, loff_t *ppos)
{
	...
	kbuf[nbytes] = '\0';
	printk(KERN_ERR "kbuf is %s\n", kbuf);
	sscanf(kbuf, "%d", &on);
	if (on) {
		printk(KERN_ERR "power on %d", on);
	} else {
		printk(KERN_ERR "power on %d", on);
	}
...
static ssize_t tc358749xbg_debugfs_reg_write(struct file *file,
					const char __user *buf,
					size_t count, loff_t *ppos)
{
	struct tc_camera_module *cam_mod = ((struct seq_file *)file->private_data)->private;
	...
	kbuf[nbytes] = '\0';
	printk(KERN_ERR "kbuf is %s\n", kbuf);
	sscanf(kbuf, " %c %x %x %d", &rw, &reg, &reg_value, &len);

	if(rw == 'w') {
		if (len == 1) {
			tc_camera_module_write8_reg(cam_mod, reg, reg_value);
			printk(KERN_ERR "%s(%d): write reg 0x%02x ---> 0x%x!\n",
				__func__, __LINE__, reg, reg_value);
		} else if (len == 2) {
			...
			printk(KERN_ERR "len %d is err!\n", len);
		}
	} else if (rw == 'r') {
		if (len == 1) {
			reg_value = tc_camera_module_read8_reg(cam_mod, reg);
			printk(KERN_ERR "%s(%d): read reg 0x%02x ---> 0x%x!\n",
				__func__, __LINE__, reg, reg_value);
		} else if (len == 2) {
		...
		printk(KERN_ERR "%c command is not support\n", rw);
	}
...
static int tc358749xbg_debugfs_reg_show(struct seq_file *s, void *v)
{
	...
	for (i = 0; i < reg_table_num_entries; i++) {
		switch(tc358749xbg_init_tab_1920_1080_30fps[i].len){
		case 1:
			value = tc_camera_module_read8_reg(cam_mod,
					tc358749xbg_init_tab_1920_1080_30fps[i].reg);
			break;
		case 2:
		...
		printk(KERN_ERR "%s(%d): reg 0x%04x ---> 0x%x\n", __func__, __LINE__,
			tc358749xbg_init_tab_1920_1080_30fps[i].reg, value);
	}
	return 0;
}
...
static const struct file_operations tc358749xbg_debug_power_fops = {
	.owner			= THIS_MODULE,
	.open			= tc358749xbg_debugfs_open,
	.read			= seq_read,
	.write			= tc358749xbg_debugfs_power_control,
	.llseek			= seq_lseek,
	...
	const struct i2c_device_id *id)
{
	...
	test_parse_dts(client);
	debugfs_dir = debugfs_create_dir("hdmiin", NULL);
	if (IS_ERR(debugfs_dir))
		printk(KERN_ERR "%s(%d): create debugfs dir failed!\n", __func__, __LINE__);
	else {
		debugfs_create_file("power_control", S_IRUSR, debugfs_dir, &tc358749xbg, &tc358749xbg_debug_power_fops);
		debugfs_create_file("register", S_IRUSR, debugfs_dir, &tc358749xbg, &tc358749xbg_debugfs_fops );
	}
