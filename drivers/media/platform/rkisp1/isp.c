				...
				struct cifisp_stat_buffer *pbuf)
{
	...
	const struct rkisp1_device *cif_dev =
	    container_of(isp_sdev, struct rkisp1_device, isp_sdev);
	enum cif_isp10_pix_fmt in_pix_fmt;

	in_pix_fmt = cif_dev->isp_sdev.isp_config.input.pix_fmt;
				...
				quantization)
{
	...
	if ((quantization != RKISP1_QUANTIZATION_FULL_RANGE) ||
...
{
	int ret;
	struct cif_isp10_vdev_node *node = vdev_to_node(vdev_cifisp);
