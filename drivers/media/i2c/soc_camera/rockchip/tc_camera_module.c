...
int tc_camera_module_s_stream(struct v4l2_subdev *sd, int enable)
{
	...
	pltfrm_camera_module_pr_err(&cam_mod->sd, "%d\n", enable);
				       ...
				       u32 *value, u32 len)
{
	...
	if (ret >= 0) {
/*		pltfrm_camera_module_pr_debug(sd,
			"ret %d,reg 0x%x data[0~3] 0x%02x, 0x%02x, 0x%02x, 0x%02x\n", ret,reg,
	...
	int len)
{
	...
	for (i = 0; i < len; i++) {
		...
		case TC_CAMERA_MODULE_REG_TYPE_DATA:
			...
			case TC_CAMERA_MOUDLE_REG_VALUE_LEN_32BIT:
				...
				break;
			default:
				pltfrm_camera_module_pr_err(&cam_mod->sd, "unknown value len, dafault 8bit\n");
				tc_camera_module_write8_reg(cam_mod, reglist[i].reg, (u8)reglist[i].val);
				break;
			}
			break;
	...
	pltfrm_camera_module_pr_err(&cam_mod->sd, "write reg list len %d\n", len);
	return 0;
}
