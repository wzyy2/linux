...
static struct cif_isp10_stream_subdev *sd_to_pathdev(struct v4l2_subdev *subdev)
{
	return container_of(subdev, struct cif_isp10_stream_subdev, subdev);
}
...
static u32 cifisp_stream_find_format(enum cif_isp10_stream_id id, const u32 pad,
				   ...
				   bool async)
{
	...
	unsigned int dc_ctrl = ioread32(dc_ctrl_addr);
	...
	iowrite32(ioread32(dc_ctrl_addr) | dc_ctrl, dc_ctrl_addr);
}

static int rkisp1_config_dcrop(struct rkisp1_device *dev,
			       enum cif_isp10_stream_id stream_id, bool async)
{
	struct cif_isp10_dcrop_config *dcrop;
	struct v4l2_mbus_framefmt *input_fmt;
	unsigned int dc_ctrl =
	    ioread32(dev->config.base_addr + CIF_DUAL_CROP_CTRL);
	...
	if (stream_id == RKISP1_STREAM_MP) {
		...
		iowrite32(dc_ctrl, dev->config.base_addr + CIF_DUAL_CROP_CTRL);
				 ...
				 enum cif_isp10_stream_id stream_id, bool async)
{
	...
	iowrite32(0, rsz_ctrl_addr);
	if (async){
		iowrite32(ioread32(rsz_ctrl_addr) | CIF_RSZ_CTRL_CFG_UPD,
			rsz_ctrl_addr);
	}
}
			     ...
			     enum cif_isp10_stream_id stream_id, bool async)
{
	...
	if (stream_id == RKISP1_STREAM_MP) {
		...
		/* No phase offset */
		iowrite32(0, dev->config.base_addr + CIF_MRSZ_PHASE_HY);
		iowrite32(0, dev->config.base_addr + CIF_MRSZ_PHASE_HC);
		iowrite32(0, dev->config.base_addr + CIF_MRSZ_PHASE_VY);
		iowrite32(0, dev->config.base_addr + CIF_MRSZ_PHASE_VC);
		...
		/* No phase offset */
		iowrite32(0, dev->config.base_addr + CIF_SRSZ_PHASE_HY);
		iowrite32(0, dev->config.base_addr + CIF_SRSZ_PHASE_HC);
		iowrite32(0, dev->config.base_addr + CIF_SRSZ_PHASE_VY);
		iowrite32(0, dev->config.base_addr + CIF_SRSZ_PHASE_VC);
		/* Linear interpolation */
		for (i = 0; i < 64; i++) {
			iowrite32(i,
				  dev->config.base_addr +
	...
	if (input_width_c < output_width_c) {
		...
		scale_h_c = DIV_TRUNCATE((input_width_c - 1)
					 * CIF_RSZ_SCALER_BYPASS,
					 output_width_c - 1);
		iowrite32(scale_h_c, scale_h_cb_addr);
		iowrite32(scale_h_c, scale_h_cr_addr);
	...
	if (input_height_c < output_height_c) {
		...
		rsz_ctrl |= CIF_RSZ_CTRL_SCALE_VC_ENABLE;
		iowrite32(DIV_TRUNCATE((output_height_c - 1)
				       * CIF_RSZ_SCALER_BYPASS,
				       input_height_c - 1) + 1, scale_v_c_addr);
	}

	iowrite32(rsz_ctrl, rsz_ctrl_addr);
	...
	if (stream_id == RKISP1_STREAM_MP) {
		...
		v4l2_info(&dev->v4l2_dev,
			  ...
			  "  MRSZ_PHASE_VY %d/%d\n"
			  "  MRSZ_PHASE_VC %d/%d\n",
			  ioread32(dev->config.base_addr + CIF_MRSZ_CTRL),
			  ioread32(dev->config.base_addr + CIF_MRSZ_CTRL_SHD),
			  ioread32(dev->config.base_addr + CIF_MRSZ_SCALE_HY),
			  ioread32(dev->config.base_addr +
				   CIF_MRSZ_SCALE_HY_SHD),
			  ioread32(dev->config.base_addr + CIF_MRSZ_SCALE_HCB),
			  ioread32(dev->config.base_addr +
				   CIF_MRSZ_SCALE_HCB_SHD),
			  ioread32(dev->config.base_addr + CIF_MRSZ_SCALE_HCR),
			  ioread32(dev->config.base_addr +
				   CIF_MRSZ_SCALE_HCR_SHD),
			  ioread32(dev->config.base_addr + CIF_MRSZ_SCALE_VY),
			  ioread32(dev->config.base_addr +
				   CIF_MRSZ_SCALE_VY_SHD),
			  ioread32(dev->config.base_addr + CIF_MRSZ_SCALE_VC),
			  ioread32(dev->config.base_addr +
				   CIF_MRSZ_SCALE_VC_SHD),
			  ioread32(dev->config.base_addr + CIF_MRSZ_PHASE_HY),
			  ioread32(dev->config.base_addr +
				   CIF_MRSZ_PHASE_HY_SHD),
			  ioread32(dev->config.base_addr + CIF_MRSZ_PHASE_HC),
			  ioread32(dev->config.base_addr +
				   CIF_MRSZ_PHASE_HC_SHD),
			  ioread32(dev->config.base_addr + CIF_MRSZ_PHASE_VY),
			  ioread32(dev->config.base_addr +
				   CIF_MRSZ_PHASE_VY_SHD),
			  ioread32(dev->config.base_addr + CIF_MRSZ_PHASE_VC),
			  ioread32(dev->config.base_addr +
		...
		v4l2_info(&dev->v4l2_dev,
			  ...
			  "  SRSZ_PHASE_VY %d/%d\n"
			  "  SRSZ_PHASE_VC %d/%d\n",
			  ioread32(dev->config.base_addr + CIF_SRSZ_CTRL),
			  ioread32(dev->config.base_addr + CIF_SRSZ_CTRL_SHD),
			  ioread32(dev->config.base_addr + CIF_SRSZ_SCALE_HY),
			  ioread32(dev->config.base_addr +
				   CIF_SRSZ_SCALE_HY_SHD),
			  ioread32(dev->config.base_addr + CIF_SRSZ_SCALE_HCB),
			  ioread32(dev->config.base_addr +
				   CIF_SRSZ_SCALE_HCB_SHD),
			  ioread32(dev->config.base_addr + CIF_SRSZ_SCALE_HCR),
			  ioread32(dev->config.base_addr +
				   CIF_SRSZ_SCALE_HCR_SHD),
			  ioread32(dev->config.base_addr + CIF_SRSZ_SCALE_VY),
			  ioread32(dev->config.base_addr +
				   CIF_SRSZ_SCALE_VY_SHD),
			  ioread32(dev->config.base_addr + CIF_SRSZ_SCALE_VC),
			  ioread32(dev->config.base_addr +
				   CIF_SRSZ_SCALE_VC_SHD),
			  ioread32(dev->config.base_addr + CIF_SRSZ_PHASE_HY),
			  ioread32(dev->config.base_addr +
				   CIF_SRSZ_PHASE_HY_SHD),
			  ioread32(dev->config.base_addr + CIF_SRSZ_PHASE_HC),
			  ioread32(dev->config.base_addr +
				   CIF_SRSZ_PHASE_HC_SHD),
			  ioread32(dev->config.base_addr + CIF_SRSZ_PHASE_VY),
			  ioread32(dev->config.base_addr +
				   CIF_SRSZ_PHASE_VY_SHD),
			  ioread32(dev->config.base_addr + CIF_SRSZ_PHASE_VC),
			  ioread32(dev->config.base_addr +
...
{
	u32 bus_code;
	struct cif_isp10_stream_subdev *path_dev =
	    (struct cif_isp10_stream_subdev *)v4l2_get_subdevdata(sd);
...
static void
cifisp_isp_stream_init_default_fmt(struct cif_isp10_stream_subdev *path_dev)
{
			   ...
			   struct v4l2_device *v4l2_dev)
{
	...
	struct cif_isp10_stream_subdev *strm_subdev = sd_to_pathdev(subdev);
