...
/* Demosaic */
#define CIFISP_BDM_BYPASS_EN(x)          ((x) << 10)
/* HIST */
#define CIFISP_HIST_PREDIV_SET(x)        ((x) << 3)
...
/* AWB */
#define CIFISP_AWB_GAIN_R_SET(x)   ((x) << 16)
#define CIFISP_AWB_GAIN_R_READ(x)  ((x) >> 16)
#define CIFISP_AWB_GAIN_B_READ(x)  ((x) & 0xFFFF)

#define CIFISP_AWB_YMAX_CMP_EN       BIT(2)
#define CIFISP_AWB_REF_CR_SET(x)   ((x) << 8)
#define CIFISP_AWB_REF_CR_READ(x)  ((x) >> 8)
#define CIFISP_AWB_REF_CB_READ(x)  ((x) & 0xFF)
#define CIFISP_AWB_MAX_CS_SET(x)   ((x) << 8)
#define CIFISP_AWB_MAX_CS_READ(x)  (((x) >> 8) & 0xFF)
#define CIFISP_AWB_MIN_C_READ(x)   ((x) & 0xFF)
#define CIFISP_AWB_MIN_Y_SET(x)    ((x) << 16)
#define CIFISP_AWB_MIN_Y_READ(x)   (((x) >> 16) & 0xFF)
#define CIFISP_AWB_MAX_Y_SET(x)    ((x) << 24)
#define CIFISP_AWB_MAX_Y_READ(x)   ((x) >> 24)
#define CIFISP_AWB_MODE_RGB_EN       ((1 << 31) | (0x02 << 0))
#define CIFISP_AWB_MODE_YCBCR_EN     ((0 << 31) | (0x02 << 0))
#define CIFISP_AWB_MODE_READ(x)    ((x) & 3)
#define CIFISP_AWB_YMAX_READ(x)    (((x) >> 2) & 1)

#define CIFISP_AWB_GET_MEAN_CR(x)   ((x) & 0xFF)
#define CIFISP_AWB_GET_MEAN_CB(x)   (((x) >> 8) & 0xFF)
#define CIFISP_AWB_GET_MEAN_Y(x)    (((x) >> 16) & 0xFF)
#define CIFISP_AWB_GET_MEAN_R(x)    ((x) & 0xFF)
#define CIFISP_AWB_GET_MEAN_B(x)    (((x) >> 8) & 0xFF)
#define CIFISP_AWB_GET_MEAN_G(x)    (((x) >> 16) & 0xFF)
#define CIFISP_AWB_GET_PIXEL_CNT(x) ((x) & 0x3FFFFFF)
...
#define CIFISP_EXP_CTRL_AUTOSTOP(x)      ((x) << 1)
#define CIFISP_EXP_CTRL_MEASMODE(x)      ((x) << 31)
#define CIFISP_EXP_HSIZE(x)              ((x) & 0x7FF)
#define CIFISP_EXP_VSIZE(x)              ((x) & 0x7FE)
/* LSC */
#define CIFISP_LSC_GRADH_SET(x)          ((x) << 11)
#define CIFISP_LSC_SECTH_SET(x)          ((x) << 10)

/* FLT */
#define CIFISP_FLT_MODE_MAX	           1
#define CIFISP_FLT_CHROMA_MODE_MAX	   3
#define CIFISP_FLT_GREEN_STAGE1_MAX	   8
#define CIFISP_FLT_MODE(x)	           ((x) << 1)
#define CIFISP_FLT_CHROMA_V_MODE(x)	   ((x) << 4)
#define CIFISP_FLT_CHROMA_H_MODE(x)	   ((x) << 6)
#define CIFISP_FLT_GREEN_STAGE1(x)	   ((x) << 8)
...
#define CIFISP_BLS_DIS           0
#define CIFISP_BLS_MODE_MEASURED BIT(1)
#define CIFISP_BLS_MODE_FIXED    0
...
#define CIFISP_CPROC_TONE_Y(x)         ((x) << 16)
#define CIFISP_CPROC_TONE_C(x)         ((x))
#define CIFISP_CPROC_TONE_Y_READ(x)    ((x) >> 16)
#define CIFISP_CPROC_TONE_C_READ(x)    ((x) & 0xFFFF)
...
#define CIFISP_AFC_WINDOW_X(x)    ((x) << 16)
#define CIFISP_AFC_WINDOW_Y(x)    (x)
...
#define CIFISP_DPF_MODE_NLL_SEGMENTATION(x)   ((x) << 6)
#define CIFISP_DPF_MODE_RB_FLTSIZE(x)         ((x) << 5)
...
#define cifisp_iowrite32(d, a)	iowrite32((u32)(d), isp_dev->base_addr + (a))
#define cifisp_ioread32(a)		ioread32(isp_dev->base_addr + (a))
#define cifisp_iowrite32OR(d, a) \
	iowrite32((ioread32(isp_dev->base_addr + (a)) | (u32)(d)), \
		(isp_dev->base_addr + (a)));
...
#define CIFISP_MEAS_MODULES (CIFISP_MODULE_AWB | CIFISP_MODULE_AEC | \
			     CIFISP_MODULE_AFC | CIFISP_MODULE_HST)

static int cifisp_module_enabled(struct cif_isp10_isp_dev *isp_dev,
				 unsigned int module)
{
	if (module & CIFISP_MEAS_MODULES)
		return ! !(isp_dev->meas_cfgs.curr->module_ens & module);
	else
		return ! !(isp_dev->other_cfgs.curr->module_ens & module);
}

static void cifisp_enable_module(struct cif_isp10_isp_dev *isp_dev,
				 unsigned int module, int enable)
{
	unsigned int *updates, *new_ens;
	unsigned long lock_flags = 0;

	spin_lock_irqsave(&isp_dev->config_lock, lock_flags);

	if (cifisp_module_enabled(isp_dev, module) == enable)
		goto out;
			     ...
			     bool flag, struct cifisp_dpcc_config *arg)
{
	...
	if (!arg) {
		CIFISP_DPRINT(CIFISP_ERROR, "arg is NULL: %s\n", __func__);
	...
	if (flag == _GET_) {
		memcpy(arg,
		       &isp_dev->other_cfgs.curr->dpcc_config, sizeof(*arg));
		goto end;
	}
	...
	if (memcmp(arg,
		   &isp_dev->other_cfgs.curr->dpcc_config, sizeof(*arg)) == 0) {
	...
	memcpy(&isp_dev->other_cfgs.last_or_new->dpcc_config,
	       arg, sizeof(struct cifisp_dpcc_config));
	isp_dev->other_cfgs.module_updates |= CIFISP_MODULE_DPCC;
end:
	spin_unlock_irqrestore(&isp_dev->config_lock, lock_flags);
			    ...
			    bool flag, struct cifisp_bls_config *arg)
{
	...
	if (flag == _GET_) {
		memcpy(arg,
		       &isp_dev->other_cfgs.curr->bls_config, sizeof(*arg));
		goto end;
	}
	...
	if (memcmp(arg,
		   &isp_dev->other_cfgs.curr->bls_config, sizeof(*arg)) == 0) {
		CIFISP_DPRINT(CIFISP_DEBUG,
			      "same param in function: %s\n", __func__);
		goto end;
	}
	...
	memcpy(&isp_dev->other_cfgs.last_or_new->bls_config,
	       arg, sizeof(struct cifisp_bls_config));
	isp_dev->other_cfgs.module_updates |= CIFISP_MODULE_BLS;
end:
	spin_unlock_irqrestore(&isp_dev->config_lock, lock_flags);
			    ...
			    bool flag, struct cifisp_lsc_config *arg)
{
	...
	if (flag == _GET_) {
		memcpy(arg,
		       &isp_dev->other_cfgs.curr->lsc_config, sizeof(*arg));
		goto end;
	}
	...
	if (memcmp(arg,
		   &isp_dev->other_cfgs.curr->lsc_config, sizeof(*arg)) == 0) {
		CIFISP_DPRINT(CIFISP_DEBUG,
			      "same param in function: %s\n", __func__);
		goto end;
	}
	...
	memcpy(&isp_dev->other_cfgs.last_or_new->lsc_config,
	       arg, sizeof(struct cifisp_lsc_config));
	isp_dev->other_cfgs.module_updates |= CIFISP_MODULE_LSC;
end:
	spin_unlock_irqrestore(&isp_dev->config_lock, lock_flags);
			    ...
			    bool flag, struct cifisp_flt_config *arg)
{
	...
	if (flag == _GET_) {
		memcpy(arg,
		       &isp_dev->other_cfgs.curr->flt_config, sizeof(*arg));
		goto end;
	}
	...
	if (memcmp(arg,
		   &isp_dev->other_cfgs.curr->flt_config, sizeof(*arg)) == 0) {
		CIFISP_DPRINT(CIFISP_DEBUG,
			      "same param in function: %s\n", __func__);
		goto end;
	}
	...
	memcpy(&isp_dev->other_cfgs.last_or_new->flt_config,
	       arg, sizeof(struct cifisp_flt_config));
	isp_dev->other_cfgs.module_updates |= CIFISP_MODULE_FLT;
end:
	spin_unlock_irqrestore(&isp_dev->config_lock, lock_flags);
			    ...
			    bool flag, struct cifisp_bdm_config *arg)
{
	...
	if (flag == _GET_) {
		memcpy(arg,
		       &isp_dev->other_cfgs.curr->bdm_config, sizeof(*arg));
		goto end;
	}
	...
	if (memcmp(arg,
		   &isp_dev->other_cfgs.curr->bdm_config, sizeof(*arg)) == 0) {
		CIFISP_DPRINT(CIFISP_DEBUG,
			      "same param in function: %s\n", __func__);
		goto end;
	}
	...
	memcpy(&isp_dev->other_cfgs.last_or_new->bdm_config,
	       arg, sizeof(struct cifisp_bdm_config));
	isp_dev->other_cfgs.module_updates |= CIFISP_MODULE_BDM;
end:
	spin_unlock_irqrestore(&isp_dev->config_lock, lock_flags);
			    ...
			    bool flag, struct cifisp_sdg_config *arg)
{
	...
	if (flag == _GET_) {
		memcpy(arg,
		       &isp_dev->other_cfgs.curr->sdg_config, sizeof(*arg));
		goto end;
	}
	...
	if (memcmp(arg,
		   &isp_dev->other_cfgs.curr->sdg_config, sizeof(*arg)) == 0) {
		CIFISP_DPRINT(CIFISP_DEBUG,
			      "same param in function: %s\n", __func__);
		goto end;
	}
	...
	memcpy(&isp_dev->other_cfgs.last_or_new->sdg_config,
	       arg, sizeof(struct cifisp_sdg_config));
	isp_dev->other_cfgs.module_updates |= CIFISP_MODULE_SDG;
end:
	spin_unlock_irqrestore(&isp_dev->config_lock, lock_flags);
			    ...
			    bool flag, struct cifisp_goc_config *arg)
{
	...
	if (flag == _GET_) {
		memcpy(arg,
		       &isp_dev->other_cfgs.curr->goc_config, sizeof(*arg));
		goto end;
	}
	...
	if (memcmp(arg,
		   &isp_dev->other_cfgs.curr->goc_config, sizeof(*arg)) == 0) {
		CIFISP_DPRINT(CIFISP_DEBUG,
			      "same param in function: %s\n", __func__);
		goto end;
	}
	...
	memcpy(&isp_dev->other_cfgs.last_or_new->goc_config,
	       arg, sizeof(struct cifisp_goc_config));
	isp_dev->other_cfgs.module_updates |= CIFISP_MODULE_GOC;
end:
	spin_unlock_irqrestore(&isp_dev->config_lock, lock_flags);
			    ...
			    bool flag, struct cifisp_ctk_config *arg)
{
	...
	if (flag == _GET_) {
		memcpy(arg,
		       &isp_dev->other_cfgs.curr->ctk_config, sizeof(*arg));
		goto end;
	}
	...
	if (memcmp(arg,
		   &isp_dev->other_cfgs.curr->ctk_config, sizeof(*arg)) == 0) {
		CIFISP_DPRINT(CIFISP_DEBUG,
			      "same param in function: %s\n", __func__);
		goto end;
	}
	...
	memcpy(&isp_dev->other_cfgs.last_or_new->ctk_config,
	       arg, sizeof(struct cifisp_ctk_config));
	isp_dev->other_cfgs.module_updates |= CIFISP_MODULE_CTK;
end:
	spin_unlock_irqrestore(&isp_dev->config_lock, lock_flags);
				 ...
				 bool flag, struct cifisp_awb_meas_config *arg)
{
	...
	if (flag == _GET_) {
		memcpy(arg,
		       &isp_dev->meas_cfgs.curr->awb_meas_config, sizeof(*arg));
		goto end;
	}
	...
	isp_dev->meas_cfgs.module_updates |= CIFISP_MODULE_AWB;
end:
	spin_unlock_irqrestore(&isp_dev->config_lock, lock_flags);
				 ...
				 bool flag, struct cifisp_awb_gain_config *arg)
{
	...
	memcpy(&isp_dev->other_cfgs.last_or_new->awb_gain_config,
	       arg, sizeof(struct cifisp_awb_gain_config));
	isp_dev->other_cfgs.module_updates |= CIFISP_MODULE_AWB_GAIN;
end:
	spin_unlock_irqrestore(&isp_dev->config_lock, lock_flags);
			    ...
			    bool flag, struct cifisp_aec_config *arg)
{
	...
	if (flag == _GET_) {
		memcpy(arg, &isp_dev->meas_cfgs.curr->aec_config, sizeof(*arg));
		goto end;
	}
	...
	if (memcmp(arg,
		   &isp_dev->meas_cfgs.curr->aec_config, sizeof(*arg)) == 0) {
		CIFISP_DPRINT(CIFISP_DEBUG,
			      "same param in function: %s\n", __func__);
		goto end;
	}
	...
	memcpy(&isp_dev->meas_cfgs.last_or_new->aec_config,
	       arg, sizeof(struct cifisp_aec_config));
	isp_dev->meas_cfgs.module_updates |= CIFISP_MODULE_AEC;
end:
	spin_unlock_irqrestore(&isp_dev->config_lock, lock_flags);
			      ...
			      bool flag, struct cifisp_cproc_config *arg)
{
	...
	if (flag == _GET_) {
		memcpy(arg,
		       &isp_dev->other_cfgs.curr->cproc_config, sizeof(*arg));
		goto end;
	}
	...
	memcpy(&isp_dev->other_cfgs.last_or_new->cproc_config,
	       arg, sizeof(struct cifisp_cproc_config));
	isp_dev->other_cfgs.module_updates |= CIFISP_MODULE_CPROC;
end:
	spin_unlock_irqrestore(&isp_dev->config_lock, lock_flags);
			    ...
			    bool flag, struct cifisp_hst_config *arg)
{
	...
	if (flag == _GET_) {
		memcpy(arg, &isp_dev->meas_cfgs.curr->hst_config, sizeof(*arg));
		goto end;
	}

	if (memcmp(arg,
		   &isp_dev->meas_cfgs.curr->hst_config, sizeof(*arg)) == 0) {
		CIFISP_DPRINT(CIFISP_DEBUG,
			      "same param in function: %s\n", __func__);
		goto end;
	}
	...
	memcpy(&isp_dev->meas_cfgs.last_or_new->hst_config,
	       arg, sizeof(struct cifisp_hst_config));
	isp_dev->meas_cfgs.module_updates |= CIFISP_MODULE_HST;
end:
	spin_unlock_irqrestore(&isp_dev->config_lock, lock_flags);
			    ...
			    bool flag, struct cifisp_afc_config *arg)
{
	...
	if (flag == _GET_) {
		memcpy(arg, &isp_dev->meas_cfgs.curr->afc_config, sizeof(*arg));
		goto end;
	}
	...
	if (memcmp(arg,
		   &isp_dev->meas_cfgs.curr->afc_config, sizeof(*arg)) == 0) {
		CIFISP_DPRINT(CIFISP_DEBUG,
			      "same param in function: %s\n", __func__);
		goto end;
	}
	...
	memcpy(&isp_dev->meas_cfgs.last_or_new->afc_config,
	       arg, sizeof(struct cifisp_afc_config));
	isp_dev->meas_cfgs.module_updates |= CIFISP_MODULE_AFC;
end:
	spin_unlock_irqrestore(&isp_dev->config_lock, lock_flags);
			   ...
			   bool flag, struct cifisp_ie_config *arg)
{
	...
	if (flag == _GET_) {
		memcpy(arg, &isp_dev->other_cfgs.curr->ie_config, sizeof(*arg));
		goto end;
	}
	...
	if (memcmp(arg,
		   &isp_dev->other_cfgs.curr->ie_config, sizeof(*arg)) == 0) {
		CIFISP_DPRINT(CIFISP_DEBUG,
			      "same param in function: %s\n", __func__);
		goto end;
	}
	...
	memcpy(&isp_dev->other_cfgs.last_or_new->ie_config,
	       arg, sizeof(struct cifisp_ie_config));
	isp_dev->other_cfgs.module_updates |= CIFISP_MODULE_IE;
end:
	spin_unlock_irqrestore(&isp_dev->config_lock, lock_flags);
			    ...
			    bool flag, struct cifisp_dpf_config *arg)
{
	...
	if (flag == _GET_) {
		memcpy(arg,
		       &isp_dev->other_cfgs.curr->dpf_config, sizeof(*arg));
		goto end;
	}
	...
	if (memcmp(arg,
		   &isp_dev->other_cfgs.curr->dpf_config, sizeof(*arg)) == 0) {
		CIFISP_DPRINT(CIFISP_DEBUG,
			      "same param in function: %s\n", __func__);
		goto end;
	}
	...
	memcpy(&isp_dev->other_cfgs.last_or_new->dpf_config,
	       arg, sizeof(struct cifisp_dpf_config));
	isp_dev->other_cfgs.module_updates |= CIFISP_MODULE_DPF;
end:
	spin_unlock_irqrestore(&isp_dev->config_lock, lock_flags);
				     ...
				     struct cifisp_dpf_strength_config *arg)
{
	...
	memcpy(&isp_dev->other_cfgs.last_or_new->dpf_strength_config,
	       arg, sizeof(struct cifisp_dpf_strength_config));
	isp_dev->other_cfgs.module_updates |= CIFISP_MODULE_DPF_STRENGTH;
end:
	spin_unlock_irqrestore(&isp_dev->config_lock, lock_flags);
...
static bool cifisp_lsc_correct_matrix_config(struct cif_isp10_isp_dev *isp_dev)
{
	...
	for (n = 0; n < ((CIFISP_LSC_SECTORS_MAX + 1) *
		...
		for (i = 0; i < (CIFISP_LSC_SECTORS_MAX); i += 2) {
			...
			data =
			    CIFISP_LSC_TABLE_DATA(pconfig->gr_data_tbl[n + i],
						  pconfig->gr_data_tbl[n + i +
								       1]);
			cifisp_iowrite32(data, CIF_ISP_LSC_GR_TABLE_DATA);

			data =
			    CIFISP_LSC_TABLE_DATA(pconfig->gb_data_tbl[n + i],
		...
		data =
		    CIFISP_LSC_TABLE_DATA(pconfig->r_data_tbl[n +
							      CIFISP_LSC_SECTORS_MAX],
					  /* isp_dev->lsc_config.r_data_tbl[n + i] */
					  0);
		cifisp_iowrite32(data, CIF_ISP_LSC_R_TABLE_DATA);

		data =
		    CIFISP_LSC_TABLE_DATA(pconfig->gr_data_tbl[n +
							       CIFISP_LSC_SECTORS_MAX],
		...
		data =
		    CIFISP_LSC_TABLE_DATA(pconfig->gb_data_tbl[n +
							       CIFISP_LSC_SECTORS_MAX],
					  /* isp_dev->lsc_config.gr_data_tbl[n + i] */
					  0);
		cifisp_iowrite32(data, CIF_ISP_LSC_GB_TABLE_DATA);
		...
		data =
		    CIFISP_LSC_TABLE_DATA(pconfig->b_data_tbl[n +
							      CIFISP_LSC_SECTORS_MAX],
...
static bool cifisp_lsc_config(struct cif_isp10_isp_dev *isp_dev)
{
	...
	if (pconfig->config_width != isp_dev->input_width ||
		...
		CIFISP_DPRINT(CIFISP_DEBUG,
			      ...
			      isp_dev->input_width, isp_dev->input_height);
		return false;
	}

	CIFISP_DPRINT(CIFISP_DEBUG,
		      "LSC config: lsc_w %d lsc_h %d\n",
		      pconfig->config_width, pconfig->config_height);
...
{
	const struct rkisp1_isp_subdev *isp_sdev =
	    container_of(isp_dev, struct rkisp1_isp_subdev, isp_dev);
	const struct cif_fmt_info *in_fmt;

	in_fmt = &isp_sdev->in_fmt;
	if (CIF_PIX_FMT_BAYER_PAT_IS_BGGR(in_fmt)) {
	...
	} else if (CIF_PIX_FMT_BAYER_PAT_IS_GBRG(in_fmt)) {
		pbuf->params.ae.bls_val.meas_gb =
		    cifisp_ioread32(CIF_ISP_BLS_A_MEASURED);
		pbuf->params.ae.bls_val.meas_b =
		    cifisp_ioread32(CIF_ISP_BLS_B_MEASURED);
		...
		pbuf->params.ae.bls_val.meas_gr =
		    cifisp_ioread32(CIF_ISP_BLS_D_MEASURED);
	} else if (CIF_PIX_FMT_BAYER_PAT_IS_GRBG(in_fmt)) {
		pbuf->params.ae.bls_val.meas_gr =
		    cifisp_ioread32(CIF_ISP_BLS_A_MEASURED);
		pbuf->params.ae.bls_val.meas_r =
		    cifisp_ioread32(CIF_ISP_BLS_B_MEASURED);
		...
		pbuf->params.ae.bls_val.meas_gb =
		    cifisp_ioread32(CIF_ISP_BLS_D_MEASURED);
	} else if (CIF_PIX_FMT_BAYER_PAT_IS_RGGB(in_fmt)) {
		pbuf->params.ae.bls_val.meas_r =
		    cifisp_ioread32(CIF_ISP_BLS_A_MEASURED);
		pbuf->params.ae.bls_val.meas_gr =
		    cifisp_ioread32(CIF_ISP_BLS_B_MEASURED);
...
static void cifisp_flt_config(const struct cif_isp10_isp_dev *isp_dev)
{
	...
	cifisp_iowrite32(pconfig->thresh_bl0, CIF_ISP_FILT_THRESH_BL0);
	cifisp_iowrite32(pconfig->thresh_bl1, CIF_ISP_FILT_THRESH_BL1);
	cifisp_iowrite32(pconfig->thresh_sh0, CIF_ISP_FILT_THRESH_SH0);
	cifisp_iowrite32(pconfig->thresh_sh1, CIF_ISP_FILT_THRESH_SH1);
	cifisp_iowrite32(pconfig->fac_bl0, CIF_ISP_FILT_FAC_BL0);
	cifisp_iowrite32(pconfig->fac_bl1, CIF_ISP_FILT_FAC_BL1);
	cifisp_iowrite32(pconfig->fac_mid, CIF_ISP_FILT_FAC_MID);
	cifisp_iowrite32(pconfig->fac_sh0, CIF_ISP_FILT_FAC_SH0);
	cifisp_iowrite32(pconfig->fac_sh1, CIF_ISP_FILT_FAC_SH1);
	cifisp_iowrite32(pconfig->lum_weight, CIF_ISP_FILT_LUM_WEIGHT);
...
static void cifisp_awb_meas_config(const struct cif_isp10_isp_dev *isp_dev)
{
	...
	/* window offset */
	cifisp_iowrite32(pconfig->awb_wnd.v_offs, CIF_ISP_AWB_WND_V_OFFS);
	cifisp_iowrite32(pconfig->awb_wnd.h_offs, CIF_ISP_AWB_WND_H_OFFS);
	/* AWB window size */
...
static void cifisp_awb_meas_end(struct cif_isp10_isp_dev *isp_dev)
{
	...
	if (!(isp_dev->meas_cfgs.last_or_new->module_ens & CIFISP_MODULE_AWB))
		cifisp_iowrite32AND(~CIF_ISP_CTRL_ISP_AWB_ENA, CIF_ISP_CTRL);
}
				...
				struct cifisp_stat_buffer *pbuf)
{
	...
	pbuf->params.awb.awb_mean[0].cnt = CIFISP_AWB_GET_PIXEL_CNT(reg_val);
...
static void cifisp_aec_config(const struct cif_isp10_isp_dev *isp_dev)
{
	...
	block_hsize = pconfig->meas_window.h_size / CIFISP_EXP_COLUMN_NUM - 1;
	block_vsize = pconfig->meas_window.v_size / CIFISP_EXP_ROW_NUM - 1;
				...
				enum v4l2_quantization quantization)
{
	...
	if ((quantization != V4L2_QUANTIZATION_FULL_RANGE) ||
		...
		cifisp_iowrite32(~(CIF_C_PROC_YOUT_FULL |
				   CIF_C_PROC_YIN_FULL |
				   CIF_C_PROC_COUT_FULL), CIF_C_PROC_CTRL);
	} else {
		cifisp_iowrite32OR((CIF_C_PROC_YOUT_FULL |
				    CIF_C_PROC_YIN_FULL |
				    CIF_C_PROC_COUT_FULL), CIF_C_PROC_CTRL);
	}
}
...
static void cifisp_afc_config(const struct cif_isp10_isp_dev *isp_dev)
{
	...
	for (i = 0; i < num_of_win; i++) {
		cifisp_iowrite32(CIFISP_AFC_WINDOW_X(pconfig->afm_win[0].h_offs)
				 | CIFISP_AFC_WINDOW_Y(pconfig->afm_win[0].
						       v_offs),
				 CIF_ISP_AFM_LT_A + i * 8);
		cifisp_iowrite32(CIFISP_AFC_WINDOW_X
				 (pconfig->afm_win[0].h_size +
				  pconfig->afm_win[0].
				  h_offs) | CIFISP_AFC_WINDOW_Y(pconfig->
								afm_win[0].
								v_size +
								pconfig->
								afm_win[0].
...
static void cifisp_hst_config(const struct cif_isp10_isp_dev *isp_dev)
{
	...
	block_hsize = pconfig->meas_window.h_size / CIFISP_HIST_COLUMN_NUM - 1;
	block_vsize = pconfig->meas_window.v_size / CIFISP_HIST_ROW_NUM - 1;
	...
	cifisp_iowrite32(CIFISP_HIST_WEIGHT_SET(pconfig->hist_weight[0],
						pconfig->hist_weight[1],
						pconfig->hist_weight[2],
						pconfig->hist_weight[3]),
			 CIF_ISP_HIST_WEIGHT_00TO30);
	cifisp_iowrite32(CIFISP_HIST_WEIGHT_SET
			 (pconfig->hist_weight[4], pconfig->hist_weight[5],
			  pconfig->hist_weight[6], pconfig->hist_weight[7]),
			 CIF_ISP_HIST_WEIGHT_40TO21);
	cifisp_iowrite32(CIFISP_HIST_WEIGHT_SET
			 (pconfig->hist_weight[8], pconfig->hist_weight[9],
			  pconfig->hist_weight[10], pconfig->hist_weight[11]),
			 CIF_ISP_HIST_WEIGHT_31TO12);
	cifisp_iowrite32(CIFISP_HIST_WEIGHT_SET
			 (pconfig->hist_weight[12], pconfig->hist_weight[13],
			  pconfig->hist_weight[14], pconfig->hist_weight[15]),
			 CIF_ISP_HIST_WEIGHT_22TO03);
	cifisp_iowrite32(CIFISP_HIST_WEIGHT_SET
			 (pconfig->hist_weight[16], pconfig->hist_weight[17],
			  pconfig->hist_weight[18], pconfig->hist_weight[19]),
			 CIF_ISP_HIST_WEIGHT_13TO43);
	cifisp_iowrite32(CIFISP_HIST_WEIGHT_SET
			 (pconfig->hist_weight[20], pconfig->hist_weight[21],
			  pconfig->hist_weight[22], pconfig->hist_weight[23]),
			 CIF_ISP_HIST_WEIGHT_04TO34);
	cifisp_iowrite32(CIFISP_HIST_WEIGHT_SET
			 (pconfig->hist_weight[24], 0, 0, 0),
...
static void cifisp_hst_en(struct cif_isp10_isp_dev *isp_dev)
{
	...
	cifisp_iowrite32OR(pconfig->mode, CIF_ISP_HIST_PROP);
}

static void cifisp_hst_end(struct cif_isp10_isp_dev *isp_dev)
{
	/* Disable measurement */
	cifisp_iowrite32(CIFISP_HISTOGRAM_MODE_DISABLE, CIF_ISP_HIST_PROP);
...
static void cifisp_ie_en(const struct cif_isp10_isp_dev *isp_dev)
{
	...
	case V4L2_COLORFX_SET_CBCR:
		effect = RKISP1_IE_SEPIA;
		break;
	case V4L2_COLORFX_BW:
		effect = RKISP1_IE_BW;
		break;
	case V4L2_COLORFX_NEGATIVE:
		effect = RKISP1_IE_NEGATIVE;
		break;
	case V4L2_COLORFX_EMBOSS:
		effect = RKISP1_IE_EMBOSS;
		break;
	case V4L2_COLORFX_SKETCH:
		effect = RKISP1_IE_SKETCH;
		break;
	case V4L2_COLORFX_AQUA:
		effect = RKISP1_IE_C_SEL;
		break;
	case V4L2_COLORFX_NONE:
	default:
		effect = RKISP1_IE_NONE;
		break;
	}
	if (effect < RKISP1_IE_NONE) {
			      ...
			      enum v4l2_quantization quantization)
{
	...
	if ((quantization != V4L2_QUANTIZATION_FULL_RANGE) ||
	    ((pconfig->effect != V4L2_COLORFX_NONE) &&
	     (isp_dev->other_cfgs.
	      last_or_new->module_ens & CIFISP_MODULE_CPROC))) {
...
static void cifisp_dpf_config(const struct cif_isp10_isp_dev *isp_dev)
{
	...
	case CIFISP_DPF_GAIN_USAGE_NF_GAINS:
		isp_dpf_mode |=
		    CIFISP_DPF_MODE_USE_NF_GAIN | CIFISP_DPF_MODE_AWB_GAIN_COMP;
		break;
	...
	if (!pconfig->rb_flt.r_enable)
		isp_dpf_mode |= CIFISP_DPF_MODE_R_FLT_DIS;
	if (!pconfig->rb_flt.b_enable)
		isp_dpf_mode |= CIFISP_DPF_MODE_B_FLT_DIS;
	if (!pconfig->g_flt.gb_enable)
		isp_dpf_mode |= CIFISP_DPF_MODE_GB_FLT_DIS;
	if (!pconfig->g_flt.gr_enable)
		isp_dpf_mode |= CIFISP_DPF_MODE_GR_FLT_DIS;
...
{
	cifisp_iowrite32OR(CIFISP_DPF_MODE_EN, CIF_ISP_DPF_MODE);
}

static void cifisp_dpf_end(struct cif_isp10_isp_dev *isp_dev)
{
	cifisp_iowrite32AND(~CIFISP_DPF_MODE_EN, CIF_ISP_DPF_MODE);
}
...
static int cifisp_dqbuf(struct file *file, void *priv, struct v4l2_buffer *p)
{
	...
	CIFISP_DPRINT(CIFISP_DEBUG,
		      " %s: %s: p->type %d p->index %d\n",
		      ISP_VDEV_NAME, __func__, p->type, p->index);
	return vb2_core_dqbuf(&isp_dev->vb2_vidq, p,
...
static int cifisp_g_ctrl(struct file *file, void *priv, struct v4l2_control *vc)
{
	...
	case V4L2_CID_CIFISP_DPCC:
		module = CIFISP_MODULE_DPCC;
		break;
	case V4L2_CID_CIFISP_BLS:
		module = CIFISP_MODULE_BLS;
		break;
	case V4L2_CID_CIFISP_SDG:
		module = CIFISP_MODULE_SDG;
		break;
	case V4L2_CID_CIFISP_LSC:
		module = CIFISP_MODULE_LSC;
		break;
	case V4L2_CID_CIFISP_AWB_MEAS:
		module = CIFISP_MODULE_AWB;
		break;
	case V4L2_CID_CIFISP_AWB_GAIN:
		module = CIFISP_MODULE_AWB_GAIN;
		break;
	case V4L2_CID_CIFISP_FLT:
		module = CIFISP_MODULE_FLT;
		break;
	case V4L2_CID_CIFISP_BDM:
		module = CIFISP_MODULE_BDM;
		break;
	case V4L2_CID_CIFISP_CTK:
		module = CIFISP_MODULE_CTK;
		break;
	case V4L2_CID_CIFISP_GOC:
		module = CIFISP_MODULE_GOC;
		break;
	case V4L2_CID_CIFISP_HST:
		module = CIFISP_MODULE_HST;
		break;
	case V4L2_CID_CIFISP_AEC:
		module = CIFISP_MODULE_AEC;
		break;
	case V4L2_CID_CIFISP_CPROC:
		module = CIFISP_MODULE_CPROC;
		break;
	case V4L2_CID_CIFISP_AFC:
		module = CIFISP_MODULE_AFC;
		break;
	case V4L2_CID_CIFISP_IE:
		module = CIFISP_MODULE_IE;
		break;
	case V4L2_CID_CIFISP_DPF:
		module = CIFISP_MODULE_DPF;
		break;
	default:
		return -EINVAL;
	}

	vc->value = cifisp_module_enabled(isp_dev, module);
	return 0;
}
...
static int cifisp_s_ctrl(struct file *file, void *priv, struct v4l2_control *vc)
{
	...
	cifisp_enable_module(isp_dev, module, vc->value);
	return 0;
}

static long cifisp_ioctl_default(struct file *file,
				 void *fh, bool valid_prio, unsigned int cmd,
				 void *arg)
{
				...
				struct poll_table_struct *wait)
{
	...
	CIFISP_DPRINT(CIFISP_DEBUG, "Polling on vbq_stat buffer %d\n", ret);
...
/* ddl@rock-chips.com: v1.0.8 */ static int cifisp_reset(struct file *file)
{
	struct cif_isp10_isp_dev *isp_dev =
	    video_get_drvdata(video_devdata(file));
...
static void cif_isp10_vb2_queue(struct vb2_buffer *vb)
{
	...
	list_add_tail(&vb->queued_entry, &isp_dev->stat);	//TODO(zsq): don't use queued_entry
...
{
	int ret;
	struct rkisp1_vdev_node *node = vdev_to_node(vdev_cifisp);
...
static inline void cifisp_meas_config_swap(struct cif_isp10_isp_dev *isp_dev)
{
	struct cifisp_isp_meas_cfg *new_cfg;

	new_cfg = isp_dev->meas_cfgs.last_or_new;
	new_cfg->s_frame_id = isp_dev->frame_id;
	isp_dev->meas_cfgs.last_or_new = isp_dev->meas_cfgs.curr;
	isp_dev->meas_cfgs.curr = new_cfg;
}

static inline void cifisp_other_config_swap(struct cif_isp10_isp_dev *isp_dev)
{
	struct cifisp_isp_other_cfg *new_cfg;

	new_cfg = isp_dev->other_cfgs.last_or_new;
	new_cfg->s_frame_id = isp_dev->frame_id;
	isp_dev->other_cfgs.last_or_new = isp_dev->other_cfgs.curr;
			  ...
			  enum v4l2_quantization quantization)
{
	...
	if (CIF_PIX_FMT_IS_RAW_BAYER(in_fmt)) {
		if (*other_ens & CIFISP_MODULE_DPCC) {
			cifisp_dpcc_config(isp_dev);
			cifisp_dpcc_en(isp_dev);
			*other_updates &= ~CIFISP_MODULE_DPCC;
		}

		if (*other_ens & CIFISP_MODULE_LSC) {
			if (cifisp_lsc_config(isp_dev))
				*other_updates &= ~CIFISP_MODULE_LSC;
		}

		if (*other_ens & CIFISP_MODULE_BLS) {
			cifisp_bls_config(isp_dev);
			cifisp_bls_en(isp_dev);
			*other_updates &= ~CIFISP_MODULE_BLS;
		}

		if (*other_ens & CIFISP_MODULE_SDG) {
			cifisp_sdg_config(isp_dev);
			cifisp_sdg_en(isp_dev);
			*other_updates &= ~CIFISP_MODULE_SDG;
		}

		if (*other_ens & CIFISP_MODULE_GOC) {
			cifisp_goc_config(isp_dev);
			cifisp_goc_en(isp_dev);
			*other_updates &= ~CIFISP_MODULE_GOC;
		}

		if (*other_ens & CIFISP_MODULE_BDM) {
			cifisp_bdm_config(isp_dev);
			cifisp_bdm_en(isp_dev);
			*other_updates &= ~CIFISP_MODULE_BDM;
		}

		if (*other_ens & CIFISP_MODULE_FLT) {
			cifisp_flt_config(isp_dev);
			cifisp_flt_en(isp_dev);
			*other_updates &= ~CIFISP_MODULE_FLT;
		}

		if (*other_ens & CIFISP_MODULE_AWB_GAIN) {
			cifisp_awb_gain_config(isp_dev);
			cifisp_awb_gain_en(isp_dev);
			*other_updates &= ~CIFISP_MODULE_AWB_GAIN;
		}

		if (*other_ens & CIFISP_MODULE_CTK) {
			cifisp_ctk_config(isp_dev);
			cifisp_ctk_en(isp_dev);
			*other_updates &= ~CIFISP_MODULE_CTK;
		}

		if (*other_ens & CIFISP_MODULE_CPROC) {
			cifisp_cproc_config(isp_dev, quantization);
			cifisp_cproc_en(isp_dev);
			*other_updates &= ~CIFISP_MODULE_CPROC;
		}

		if (*other_ens & CIFISP_MODULE_IE) {
			cifisp_ie_config(isp_dev);
			cifisp_ie_en(isp_dev);
			*other_updates &= ~CIFISP_MODULE_IE;
		}

		cifisp_csm_config(isp_dev, quantization);

		if (*other_ens & CIFISP_MODULE_DPF) {
			cifisp_dpf_config(isp_dev);
			cifisp_dpf_strength_config(isp_dev);
			cifisp_dpf_en(isp_dev);
			*other_updates &= ~(CIFISP_MODULE_DPF |
					    CIFISP_MODULE_DPF_STRENGTH);
		}

		if (*meas_ens & CIFISP_MODULE_AFC) {
			cifisp_afc_config(isp_dev);
			cifisp_afc_en(isp_dev);
			*meas_updates &= ~CIFISP_MODULE_AFC;
		}

		if (*meas_ens & CIFISP_MODULE_AWB) {
			cifisp_awb_meas_config(isp_dev);
			cifisp_awb_meas_en(isp_dev);
			*meas_updates &= ~CIFISP_MODULE_AWB;
		}

		if (*meas_ens & CIFISP_MODULE_AEC) {
			cifisp_aec_config(isp_dev);
			cifisp_aec_en(isp_dev);
			*meas_updates &= ~CIFISP_MODULE_AEC;
		}

		if (*meas_ens & CIFISP_MODULE_HST) {
			cifisp_hst_config(isp_dev);
			cifisp_hst_en(isp_dev);
			*meas_updates &= ~CIFISP_MODULE_HST;
		}
	} else {
	...
	memcpy(isp_dev->other_cfgs.last_or_new,
	       isp_dev->other_cfgs.curr, sizeof(struct cifisp_isp_other_cfg));
	memcpy(isp_dev->meas_cfgs.last_or_new,
	       isp_dev->meas_cfgs.curr, sizeof(struct cifisp_isp_meas_cfg));
...
static void cifisp_sensor_mode_data_sync(struct rkisp1_device *dev,
...
static void cifisp_send_measurement(struct cif_isp10_isp_dev *isp_dev, struct cif_isp10_isp_readout_work
				    *meas_work)
{
	unsigned long lock_flags = 0;
	...
	const struct rkisp1_isp_subdev *isp_sdev =
	    container_of(isp_dev, struct rkisp1_isp_subdev, isp_dev);
	struct rkisp1_device *cif_dev =
	    container_of(isp_sdev, struct rkisp1_device, isp_sdev);
	...
	if (isp_dev->frame_id != meas_work->frame_id) {
		...
		CIFISP_DPRINT(CIFISP_ERROR,
			      ...
			      isp_dev->frame_id, meas_work->frame_id);
		goto end;
	}
	...
	if (!list_empty(&isp_dev->stat)) {
		...
		spin_unlock_irqrestore(&isp_dev->irq_lock, lock_flags);
		CIFISP_DPRINT(CIFISP_DEBUG, "Not enought measurement bufs\n");
		goto end;
	}
	...
	stat_buf =
	    (struct cifisp_stat_buffer *)vb->vb2_queue->mem_ops->vaddr(vb->
								       planes
								       [0].
								       mem_priv);
	...
	if (active_meas & CIF_ISP_EXP_END) {
		cifisp_sensor_mode_data_sync(cif_dev, meas_work->frame_id,
	...
	CIFISP_DPRINT(CIFISP_DEBUG, "Measurement done\n");
	vb = NULL;
end:
...
void cifisp_isp_readout_work(struct work_struct *work)
{
	...
	struct cif_isp10_isp_dev *isp_dev = readout_work->isp_dev;
...
static inline bool cifisp_isp_isr_other_config(struct cif_isp10_isp_dev
					       *isp_dev,
					       unsigned int *time_left)
{
	...
	if (isp_dev->other_cfgs.module_updates & CIFISP_MODULE_DPCC) {
		/*update dpc config */
		cifisp_dpcc_config(isp_dev);

		if (*ens & CIFISP_MODULE_DPCC)
			cifisp_dpcc_en(isp_dev);
		else
			cifisp_dpcc_end(isp_dev);
	...
	if ((isp_dev->other_cfgs.module_updates &
	     CIFISP_MODULE_BLS) && *time_left >= CIFISP_MODULE_BLS_PROC_TIME) {
		/* update bls config */
		cifisp_bls_config(isp_dev);

		if (*ens & CIFISP_MODULE_BLS)
			cifisp_bls_en(isp_dev);
		else
			cifisp_bls_end(isp_dev);
		...
		*time_left -= CIFISP_MODULE_BLS_PROC_TIME;
		CIFISP_DPRINT(CIFISP_DEBUG, "bls time-left :%d\n", *time_left);
	}

	if ((isp_dev->other_cfgs.module_updates &
	     CIFISP_MODULE_SDG) && *time_left >= CIFISP_MODULE_SDG_PROC_TIME) {
		/* update sdg config */
		cifisp_sdg_config(isp_dev);

		if (*ens & CIFISP_MODULE_SDG)
			cifisp_sdg_en(isp_dev);
		else
			cifisp_sdg_end(isp_dev);
		...
		*time_left -= CIFISP_MODULE_SDG_PROC_TIME;
		CIFISP_DPRINT(CIFISP_DEBUG, "sdg time-left :%d\n", *time_left);
	}

	if ((isp_dev->other_cfgs.module_updates &
	     CIFISP_MODULE_LSC) && *time_left >= CIFISP_MODULE_LSC_PROC_TIME) {
		/* update lsc config */
		bool res = true;

		if (*ens & CIFISP_MODULE_LSC) {
			if (!cifisp_lsc_config(isp_dev))
				res = false;
		} else {
			cifisp_lsc_end(isp_dev);
		...
		*time_left -= CIFISP_MODULE_LSC_PROC_TIME;
		CIFISP_DPRINT(CIFISP_DEBUG, "lsc time-left :%d\n", *time_left);
	}
	...
	if ((isp_dev->other_cfgs.module_updates &
	     CIFISP_MODULE_BDM) && *time_left >= CIFISP_MODULE_BDM_PROC_TIME) {
		/* update bdm config */
		cifisp_bdm_config(isp_dev);

		if (*ens & CIFISP_MODULE_BDM)
			cifisp_bdm_en(isp_dev);
		else
			cifisp_bdm_end(isp_dev);
		...
		*time_left -= CIFISP_MODULE_BDM_PROC_TIME;
		CIFISP_DPRINT(CIFISP_DEBUG, "bdm time-left :%d\n", *time_left);
	}

	if ((isp_dev->other_cfgs.module_updates &
	     CIFISP_MODULE_FLT) && *time_left >= CIFISP_MODULE_FLT_PROC_TIME) {
		/* update filter config */
		cifisp_flt_config(isp_dev);

		if (*ens & CIFISP_MODULE_FLT)
			cifisp_flt_en(isp_dev);
		else
			cifisp_flt_end(isp_dev);
		...
		*time_left -= CIFISP_MODULE_FLT_PROC_TIME;
		CIFISP_DPRINT(CIFISP_DEBUG, "flt time-left :%d\n", *time_left);
	}

	if ((isp_dev->other_cfgs.module_updates &
	     CIFISP_MODULE_CTK) && *time_left >= CIFISP_MODULE_CTK_PROC_TIME) {
		/* update ctk config */
		cifisp_ctk_config(isp_dev);

		if (*ens & CIFISP_MODULE_CTK)
			cifisp_ctk_en(isp_dev);
		else
			cifisp_ctk_end(isp_dev);
	...
	if ((isp_dev->other_cfgs.module_updates &
	     CIFISP_MODULE_GOC) && *time_left >= CIFISP_MODULE_GOC_PROC_TIME) {
		/* update goc config */
		cifisp_goc_config(isp_dev);

		if (*ens & CIFISP_MODULE_GOC)
			cifisp_goc_en(isp_dev);
		else
			cifisp_goc_end(isp_dev);
		...
		*time_left -= CIFISP_MODULE_GOC_PROC_TIME;
		CIFISP_DPRINT(CIFISP_DEBUG, "goc time-left :%d\n", *time_left);
	}
	...
	if ((isp_dev->other_cfgs.module_updates &
	    ...
	    *time_left >= CIFISP_MODULE_CPROC_PROC_TIME) {
		/* update cprc config */
		cifisp_cproc_config(isp_dev, isp_dev->quantization);

		cifisp_csm_config(isp_dev, isp_dev->quantization);

		if (*ens & CIFISP_MODULE_CPROC)
			cifisp_cproc_en(isp_dev);
		else
			cifisp_cproc_end(isp_dev);
	...
	if ((isp_dev->other_cfgs.module_updates &
	     CIFISP_MODULE_IE) && *time_left >= CIFISP_MODULE_IE_PROC_TIME) {
		/* update ie config */
		cifisp_ie_config(isp_dev);

		if (*ens & CIFISP_MODULE_IE)
			cifisp_ie_en(isp_dev);
		else
			cifisp_ie_end(isp_dev);
		...
		*time_left -= CIFISP_MODULE_IE_PROC_TIME;
		CIFISP_DPRINT(CIFISP_DEBUG, "ie time-left :%d\n", *time_left);
	}

	if ((isp_dev->other_cfgs.module_updates &
	     CIFISP_MODULE_DPF) && *time_left >= CIFISP_MODULE_DPF_TIME) {
		/* update dpf  config */
		cifisp_dpf_config(isp_dev);

		if (*ens & CIFISP_MODULE_DPF)
			cifisp_dpf_en(isp_dev);
		else
			cifisp_dpf_end(isp_dev);
		...
		*time_left -= CIFISP_MODULE_DPF_TIME;
		CIFISP_DPRINT(CIFISP_DEBUG, "dpf time-left :%d\n", *time_left);
	}
...
static inline bool cifisp_isp_isr_meas_config(struct cif_isp10_isp_dev *isp_dev,
					      unsigned int *time_left)
{
	unsigned int time_in = *time_left;
	bool config_chk;
	...
	if (isp_dev->meas_cfgs.module_updates & CIFISP_MODULE_AWB) {
		/* update awb config */
		cifisp_awb_meas_config(isp_dev);

		if (*ens & CIFISP_MODULE_AWB)
			cifisp_awb_meas_en(isp_dev);
		else
			cifisp_awb_meas_end(isp_dev);
	...
	if (isp_dev->meas_cfgs.module_updates & CIFISP_MODULE_AFC) {
		/* update afc config */
		cifisp_afc_config(isp_dev);

		if (*ens & CIFISP_MODULE_AFC)
			cifisp_afc_en(isp_dev);
		else
			cifisp_afc_end(isp_dev);
	...
	if (isp_dev->meas_cfgs.module_updates & CIFISP_MODULE_HST) {
		/* update hst config */
		cifisp_hst_config(isp_dev);

		if (*ens & CIFISP_MODULE_HST)
			cifisp_hst_en(isp_dev);
		else
			cifisp_hst_end(isp_dev);
	...
	if (isp_dev->meas_cfgs.module_updates & CIFISP_MODULE_AEC) {
		/* update aec config */
		cifisp_aec_config(isp_dev);

		if (*ens & CIFISP_MODULE_AEC)
			cifisp_aec_en(isp_dev);
		else
			cifisp_aec_end(isp_dev);
...
int cifisp_isp_isr(struct cif_isp10_isp_dev *isp_dev, u32 isp_mis)
{
	...
	if (isp_mis & CIF_ISP_FRAME) {
		...
		if (((isp_dev->meas_cfgs.module_updates &
		      CIFISP_MEAS_MODULES) == 0) &&
		    isp_dev->active_meas &&
		    ((isp_dev->active_meas & isp_ris) ==
		     isp_dev->active_meas)) {
			work = (struct cif_isp10_isp_readout_work *)
			    kmalloc(sizeof(struct cif_isp10_isp_readout_work),
				    GFP_ATOMIC);
			if (work) {
	...
	case CIFISP_MODULE_SDG:
		{
			...
			CIFISP_DPRINT(CIFISP_DEBUG,
				      ...
				      ISP_VDEV_NAME);
			CIFISP_DPRINT(CIFISP_DEBUG, " RED -Curve parameters\n");
			for (i = 0; i < CIFISP_DEGAMMA_CURVE_SIZE; i++) {
				CIFISP_DPRINT(CIFISP_DEBUG,
					      " gamma_y[%d]: %d\n",
...
#ifdef LOG_CAPTURE_PARAMS
static void cifisp_reg_dump_capture(const struct
				    cif_isp10_isp_dev
				    *isp_dev)
{
...
#ifdef CIFISP_DEBUG_REG
static void cifisp_reg_dump(const struct
			    cif_isp10_isp_dev
			    *isp_dev, unsigned int module, int level)
{
	switch (module) {
	case CIFISP_MODULE_DPCC:
	...
	case CIFISP_MODULE_AWB:{
			...
			CIFISP_DPRINT(level,
				      " CIF_ISP_AWB_GAIN_G: %x\n",
				      cifisp_ioread32(CIF_ISP_AWB_GAIN_G));
			CIFISP_DPRINT(level,
				      " CIF_ISP_AWB_GAIN_RB: %x\n",
