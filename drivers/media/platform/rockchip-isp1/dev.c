...
static int register_stream_subdevs(struct rkisp1_device *dev)
{
	...
	if (ret < 0)
		return ret;
	ret = rkisp1_register_videodev(dev, RKISP1_STREAM_MP);
	if (ret < 0)
	...
	ret = rkisp1_register_videodev(dev, RKISP1_STREAM_SP);
	if (ret < 0)
...
err_unreg_mp_vdev:
	rkisp1_unregister_video_device(&dev->strm_vdevs.mp_vdev);
err_unreg_mp_subdev:
...
static void unregister_stream_subdevs(struct rkisp1_device *dev)
{
	...
	rkisp1_unregister_video_device(&dev->strm_vdevs.mp_vdev);
	unregister_stream_subdev(&dev->path_sdevs.sp_sdev.subdev);
	rkisp1_unregister_video_device(&dev->strm_vdevs.sp_vdev);
}
...
static int rkisp1_plat_probe(struct platform_device *pdev)
{
	...
	rkisp1_stream_init(isp_dev, RKISP1_ALL_STREAMS);
	spin_lock_init(&isp_dev->strm_vdevs.vbq_lock);
	spin_lock_init(&isp_dev->writel_verify_lock);
...
static int rkisp1_plat_remove(struct platform_device *pdev)
{
	...
	rkisp1_stream_release(isp_dev, RKISP1_ALL_STREAMS);
...
static int rkisp1_pm_suspend(struct device *dev)
{
	...
	struct rkisp1_stream_vdevs *strm = &isp_dev->strm_vdevs;
	int ret = 0;

	if (pm_runtime_suspended(isp_dev->dev))
		return 0;

	strm->sp_vdev.saved_state = strm->sp_vdev.state;
	strm->mp_vdev.saved_state = strm->mp_vdev.state;
	ret = rkisp1_streamoff(isp_dev, RKISP1_ALL_STREAMS);
	if (ret < 0)
		return ret;
...
{
	struct rkisp1_device *isp_dev = dev_get_drvdata(dev);
	struct rkisp1_stream *mp_vdev = &isp_dev->strm_vdevs.mp_vdev;
	struct rkisp1_stream *sp_vdev = &isp_dev->strm_vdevs.sp_vdev;
	...
	return rkisp1_streamon(isp_dev, stream_ids);
}
